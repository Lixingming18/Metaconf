...
def get_db_info(db_path):
    ...
    try:
        ...
        for table in tables:
            ...
            cursor.execute(
                f"SELECT SUM(length(title)) + SUM(length(text)) FROM {table_name};"
            )
            size_result = cursor.fetchone()
            size = size_result[0] if size_result[0] is not None else 0

            table_info[table_name] = {"columns": columns, "count": count, "size": size}
...
def process_document(
    title, text, titles, output_lines, tokenizer, token_stats, model_max_length
):
    """
    ...
    for sentence in sentences:
        ...
        if current_passage and (
            current_token_count + sentence_token_count > MAX_LENGTH
        ):
        ...
        if sentence_token_count > MAX_LENGTH:
            ...
            while start_idx < len(sentence_tokens):
                ...
                segment = tokenizer.decode(
                    sentence_tokens[start_idx:end_idx], skip_special_tokens=True
                )
...
def build_knowledge_base(
    db_path, data_path, batch_size=10000, max_docs=None, model_max_length=450
):
    """
    ...
    token_stats = {"max_count": 0, "total": 0}
    ...
    if is_json_format:
        ...
        try:
            ...
            if not isinstance(data_array, list):
                print(
                    "Warning: JSON file doesn't contain array, converting to single object array"
                )
            ...
            for i, dp in enumerate(tqdm(data_array, desc="Processing", unit="docs")):
                try:
                    ...
                    if len(output_lines) >= batch_size:
                        ...
                        tqdm.write(
                            f"Processed: {tot:,} | Skipped: {skipped:,} | Speed: {avg_speed:.1f} docs/sec"
                        )
        ...
        with open(data_path, "r", encoding="utf-8") as f:
            for line_num, line in enumerate(
                tqdm(f, total=total_lines, desc="Processing", unit="docs")
            ):
...
if __name__ == "__main__":
    ...
    print(
        f"Successfully built knowledge base with {total_docs:,} documents in {end-start:.2f}s"
    )
