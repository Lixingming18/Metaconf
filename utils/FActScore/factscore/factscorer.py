...
class FactScorer:
    ...
    def __init__(
        ...
        if dedicated_gpu_id is not None and torch.cuda.is_available():
            if torch.cuda.device_count() > dedicated_gpu_id:
                print(
                    f"Locking GPU {dedicated_gpu_id}, memory limit: {limit_memory_fraction}"
                )
                torch.cuda.set_device(dedicated_gpu_id)
                torch.cuda.set_per_process_memory_fraction(
                    limit_memory_fraction, dedicated_gpu_id
                )
        ...
        if self.use_nli:
            try:
                ...
                try:
                    ...
                    logging.error(
                        f"Failed to download 'punkt' data: {e}. Sentence splitting might be suboptimal."
                    )
    ...
    def _quantize_model(self, model=None):
        ...
        if device_to_use.type == "cuda":
            logging.warning(
                "Quantized models do not support CUDA backend. Using original non-quantized model on GPU."
            )
            return model
        ...
        try:
            ...
            logging.info(
                f"Applying {self.quantization_type} quantization (CPU mode only)..."
            )
    ...
    def _optimize_nli_model(self):
        ...
        try:
            logging.info(
                f"Initializing and optimizing NLI model: {self.nli_model_name}"
            )
            ...
            try:
                if hasattr(model.config, "label2id") and model.config.label2id:
                    self.entailment_label = next(
                        label
                        for label, id_ in model.config.label2id.items()
            ...
            logging.info(
                f"NLI model optimization completed, using device: {self.device}"
            )
            return True
    ...
    def _initialize_nli_pipeline(self, device=None):
        ...
        if self.nli_pipeline is not None:
            if (
                hasattr(self.nli_pipeline.model, "name_or_path")
                and self.nli_pipeline.model.name_or_path == self.nli_model_name
            ):
                return self.nli_pipeline
            else:
                logging.info(
                    f"Switching NLI model to '{self.nli_model_name}', reinitializing pipeline"
                )
        ...
        try:
            logging.info(
                f"Initializing NLI pipeline with model '{self.nli_model_name}'..."
            )
            tokenizer = AutoTokenizer.from_pretrained(self.nli_model_name)
            model = AutoModelForSequenceClassification.from_pretrained(
                self.nli_model_name
            ...
            if device_to_use.type == "cpu" and self.quantization_type:
                ...
                logging.warning(
                    f"Quantization not supported on CUDA device, using non-quantized model"
                )
            ...
            logging.info(
                f"NLI pipeline initialized successfully, using device: {device_to_use}"
            )
            return self.nli_pipeline
        except Exception as e:
    ...
    def _is_fact_supported_by_text_nli_optimized(
        self, atomic_fact, passage_text, use_sentence_splitting=False, batch_size=None
    ):
        """
        ...
        if use_sentence_splitting:
            try:
                ...
                sentences = [
                    s.strip() for s in re.split(r"[.?!]\s+", passage_text) if s.strip()
                ]
                if not sentences:
                    sentences = [passage_text]
        else:
        ...
        for i in range(0, len(texts), batch_size):
            ...
            with torch.no_grad():
                inputs = self.optimized_tokenizer(
                    batch_texts,
                    batch_pairs,
                    return_tensors="pt",
                    padding=True,
    ...
    def _clean_cache(self):
        ...
        if (
            hasattr(self, "sentence_cache")
            and len(self.sentence_cache) > self.max_cache_size // 2
        ):
    ...
    def _is_fact_supported_by_text_nli(
        self, atomic_fact, passage_text, use_sentence_splitting=False, device=None
    ):
        """
        ...
        if self.optimized_model is not None:
            return self._is_fact_supported_by_text_nli_optimized(
                atomic_fact, passage_text
            )
        ...
        try:
            if (
                hasattr(nli_pipeline.model.config, "label2id")
                and nli_pipeline.model.config.label2id
            ):
                entailment_label = next(
                    label
                    for label, id_ in nli_pipeline.model.config.label2id.items()
                    if label.upper() == "ENTAILMENT"
                )
            else:
        ...
        if use_sentence_splitting:
            ...
            input_pairs = [
                {"text": sentence, "text_pair": atomic_fact} for sentence in sentences
            ]
        else:
        ...
        try:
            ...
            for i, result_list in enumerate(results):
                ...
                if predicted_label == entailment_label:
                    passes_threshold = (self.nli_entailment_threshold is None) or (
                        found_entailment_score >= self.nli_entailment_threshold
                    )
    ...
    def register_knowledge_source(self, name=None, db_path=None):
        ...
        cache_path = (
            os.path.join(cache_dir, f"retrieval-{name}.json") if cache_dir else None
        )
        embed_cache_path = (
            os.path.join(cache_dir, f"retrieval-{name}.pkl") if cache_dir else None
        )
    ...
    def get_score(
        ...
        try:
            ...
            if knowledge_source not in self.retrieval:
                raise ValueError(
                    f"Knowledge source '{knowledge_source}' not registered. Please use register_knowledge_source()"
                )
            ...
            if isinstance(topics, str) and isinstance(generations, str):
                ...
                assert len(topics) == len(
                    generations
                ), "`topics` and `generations` should have same length"
            ...
            if atomic_facts is not None:
                assert len(topics) == len(
                    atomic_facts
                ), "`topics` and `atomic_facts` should have same length"
            else:
                if self.af_generator is None:
                    ...
                    cache_file = (
                        os.path.join(cache_dir, f"{self.af_model_name}.pkl")
                        if cache_dir
                        else None
                    )
                ...
                if valid_generations:
                    if self.verbose:
                        print(
                            f"Batch processing {len(valid_generations)} valid responses to generate atomic facts..."
                        )
                    ...
                    for idx, (i, facts) in enumerate(zip(valid_indices, all_afs)):
                        if not facts or len(facts) == 0:
                            ...
                            filtered_facts = [
                                fact
                                for fact in facts
                                if not fact.startswith("NON_FACTUAL:")
                            ]
            ...
            if self.verbose:
                topics_iter = tqdm(
                    list(zip(topics, generations, atomic_facts)),
                    desc="Evaluating factual accuracy",
                )
            else:
            ...
            for i, (topic, generation, facts) in enumerate(topics_iter):
                if facts is None:
                    ...
                    decision, time_dict = self._get_score(
                        topic, generation, facts, knowledge_source
                    )
                    decisions = decision
                    num_facts = len(facts)
    ...
    def _get_score(
        self, topic, generation, atomic_facts, knowledge_source, cost_estimate=None
    ):
        """
        ...
        if self.use_nli:
            ...
            for i, (atom, passages) in enumerate(zip(valid_facts, valid_passages_list)):
                passage_texts = [
                    passage["text"].replace("<s>", "").replace("</s>", "")
                ...
                for passage_text in passage_texts:
                    is_supported, confidence_score, supporting_sentence = (
                        self._is_fact_supported_by_text_nli_optimized(
                ...
                if self.fact_counter % self.deep_clean_interval == 0:
                    if self.verbose:
                        print(
                            f"Performing deep cleanup at atomic fact #{self.fact_counter}..."
                        )
            ...
            for atom, passages in zip(valid_facts, valid_passages_list):
                ...
                decisions.append(
                    {"atom": atom, "is_supported": is_supported, "passages": passages}
                )
    ...
    def _get_score_batched(
        self, topic, generation, atomic_facts, knowledge_source, chunk_size=100
    ):
...
def main():
    parser = argparse.ArgumentParser(
        description="FactScore - Evaluate factual accuracy of generated text"
    )
    ...
    parser.add_argument(
        "--db_path", type=str, default=None, help="Knowledge base database path"
    )

    parser.add_argument(
        "--gamma", type=int, default=10, help="Length penalty hyperparameter"
    )
    ...
    parser.add_argument(
        "--use_atomic_facts", action="store_true", help="Use pre-generated atomic facts"
    )

    parser.add_argument(
        "--verbose", action="store_true", help="Show detailed output and progress bars"
    )
    ...
    with open(args.input_path) as f:
        for line in f:
            ...
            if args.use_atomic_facts:
                assert (
                    "annotations" in dp
                ), "You can only specify `--use_atomic_facts` when atomic facts are already in input data"
    ...
    if isinstance(results, list):
        for i, result in enumerate(results):
            if args.count_supported:
                logging.critical(
                    f"Generation {i+1} FActScore (supported fact count) = {result['score']:.1f}"
                )
            else:
                logging.critical(
                    f"Generation {i+1} FActScore = {100 * result['score']:.1f}%"
                )
                if "init_score" in result:
                    logging.critical(
                        f"Generation {i+1} FActScore (no length penalty) = {100 * result['init_score']:.1f}%"
                    )
            logging.critical(
                f"Generation {i+1} atomic fact count = {result['num_facts_per_response']:.1f}"
            )
            logging.critical(
                f"Generation {i+1} method: {'NLI' if result['using_nli'] else 'LLM'}"
            )
    else:
        result = results
        if args.count_supported:
            logging.critical(
                f"FActScore (supported fact count) = {result['score']:.1f}"
            )
        else:
            logging.critical(f"FActScore = {100 * result['score']:.1f}%")
            if "init_score" in result:
                logging.critical(
                    f"FActScore (no length penalty) = {100 * result['init_score']:.1f}%"
                )
        logging.critical(f"Response ratio = {100 * result['respond_ratio']:.1f}%")
        logging.critical(
            f"Atomic facts per valid response = {result['num_facts_per_response']:.1f}"
        )
