    ...
    def __init__(self, db_path=None):
        ...
        with tqdm(total=100, desc="Loading database", ncols=80) as pbar:
            ...
            if len(cursor.fetchall()) == 0:
                self.close()
                raise ValueError(
                    f"Database {self.db_path} is empty, contains no tables"
                )
...
class Retrieval(object):
    def __init__(
        ...
        if retrieval_type != "bm25" and not retrieval_type.startswith("gtr-"):
            raise ValueError(
                f"Unsupported retrieval type: {retrieval_type}, must be bm25 or start with gtr-"
            )
        ...
        LogConfig.log(
            f"Retrieval initialized, type: {retrieval_type}", level="info", force=True
        )
    ...
    def load_cache(self):
        def _load_cache_file():
            if self.cache_path and os.path.exists(self.cache_path):
                LogConfig.log(
                    f"Loading retrieval cache from {self.cache_path}...", level="info"
                )
                try:
                    ...
                    LogConfig.log(
                        f"Failed to load retrieval cache: {str(e)}",
                        level="warning",
                        force=True,
                    )
            return {}
        ...
        def _load_embed_cache_file():
            if self.embed_cache_path and os.path.exists(self.embed_cache_path):
                LogConfig.log(
                    f"Loading embedding cache from {self.embed_cache_path}...",
                    level="info",
                )
                try:
                    ...
                    LogConfig.log(
                        f"Failed to load embedding cache: {str(e)}",
                        level="warning",
                        force=True,
                    )
            return {}
        ...
        elapsed = time.time() - start_time
        LogConfig.log(
            f"Cache loaded, retrieval: {len(self.cache)} items, embedding: {len(self.embed_cache)} items, time: {elapsed:.2f}s",
            level="info",
        )

    def save_cache(self):
        if (self.add_n == 0 and self.add_n_embed == 0) or (
            not self.cache_path and not self.embed_cache_path
        ):
            return
        ...
        def _save_cache_file():
            if self.add_n > 0 and self.cache_path:
                LogConfig.log(
                    f"Saving {self.add_n} new retrieval cache items...", level="info"
                )
        ...
        def _save_embed_cache_file():
            if self.add_n_embed > 0 and self.embed_cache_path:
                LogConfig.log(
                    f"Saving {self.add_n_embed} new embedding cache items...",
                    level="info",
                )
    ...
    def get_gtr_passages(self, topic, retrieval_query, passages, k):
        ...
        if topic in self.embed_cache:
            ...
            passage_vectors = np.zeros(
                (len(inputs), self.encoder.get_sentence_embedding_dimension()),
                dtype=np.float32,
            )
            ...
            with concurrent.futures.ThreadPoolExecutor() as executor:
                futures = [
                    executor.submit(encode_batch, batch_idx)
    ...
    def get_passages(self, topic, question, k=5):
        ...
        if cache_key in self.cache:
            LogConfig.log(
                f"Using cached retrieval result (topic: '{topic}')", level="debug"
            )
            return self.cache[cache_key]

        LogConfig.log(
            f"Retrieving... topic: '{topic}', question: '{question}'", level="info"
        )
