...
class LM(object):
    ...
    def generate(
        self, prompt, sample_idx=0, max_sequence_length=4096, max_output_length=4096
    ):
        prompt = prompt.strip()
    ...
    def generate_batch(
        ...
        print(
            f"{len(generated_batch)} cached, running inference for {len(inference_batch)} items."
        )
        ...
        generated_batch_list = [
            generated_batch[i] for i in range(len(prompt_batch)) if i in generated_batch
        ]
    ...
    def _generate_batch(
        self, prompt_batch, max_sequence_length=4096, max_output_length=4096
    ):
        generated_texts = []
        generated_arrs = []
    ...
    def load_cache(self, allow_retry=True):
        ...
        print(
            f"(LM) Loaded {len(cache)} cache items in {time.time() - start_time:.3f} seconds."
        )
        return cache
